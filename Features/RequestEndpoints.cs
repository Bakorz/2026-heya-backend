...
public static class RequestEndpoints
{
    ...
    public static RouteGroupBuilder MapRequestEndpoints(this RouteGroupBuilder api)
    {
        ...
        group.MapPost("", async (AppDbContext db, CreateBookingRequest request) =>
        {
            var room = await db.Rooms.FirstOrDefaultAsync(x => x.Id == request.RoomId && x.IsActive);
            if (room is null)
            {
                return Results.BadRequest("Room not found or inactive.");
            }

            if (request.AttendeeCount > room.Capacity)
            {
                return Results.BadRequest("Attendee count exceeds room capacity.");
            }
            ...
            {
                return Results.BadRequest("Unable to generate request occurrences.");
            }

            if (occurrences.Any(x => !IsWithinWorkingHours(x.StartUtc, x.EndUtc)))
            {
                return Results.BadRequest("All occurrences must be within working hours (08:00-20:00 UTC). ");
            }
            ...
            foreach (var occurrence in occurrences)
            {
                ...
                if (overlap)
                {
                    return Results.BadRequest($"Time conflict found at {occurrence.StartUtc:u} - {occurrence.EndUtc:u}.");
                }
            }
            ...
            return Results.Created($"/api/requests/{bookingRequest.Id}", bookingRequest);
        });
